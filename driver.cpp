...
void Driver::handle_leverageScores() {
	log->info("Starting truncated SVD computation");
	MatrixHandle inputMat{input.readInt()};
	uint32_t max_k = input.readInt();
	uint32_t method = input.readInt();
	...
	MatrixDescriptor Sinfo(SHandle, m, max_k);
	ENSURE(matrices.insert(std::make_pair(SHandle, Sinfo)).second);

	for (auto k = 1; k <= max_k; k++)
	{
		TruncatedSVDCommand cmd(inputMat, SHandle, k, method);
		issue(cmd);
		...
		log->info("Waiting on workers to store S");

		world.barrier();
	}
	log->info("Writing ok status followed by S handle");
	output.writeInt(0x1);
	output.writeInt(SHandle.id);
	output.flush();
}
