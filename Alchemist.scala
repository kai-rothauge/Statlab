...
class DriverClient(val istream: InputStream, val ostream: OutputStream) {
  ...
  def leverageScores(mat: MatrixHandle, k: Int, method: Int) : MatrixHandle = {
    output.writeInt(0x88)
    output.writeInt(mat.id)
    output.writeInt(k)
...
class Alchemist(val mysc: SparkContext) {
  ...
  def leverageScores(mat: AlMatrix, k: Int, method: Int = 0) : AlMatrix = {
    val sHandle = client.leverageScores(mat.handle, k, method)
    
    new AlMatrix(this, sHandle)
  }
